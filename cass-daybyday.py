...
def sumCounts(row):
    ...
    counts = row['counts']
    row['cnt'] = 666 + counts['ga_videoPlays'] + counts['sda_downloads'] + counts['fb_socialFacebookLikes'] + counts['fb_socialFacebookShares'] + counts['fb_socialFacebookComments'] + counts['tw_socialTwitterShares'] + counts['ga_socialGooglePlusShares'] + counts['gigya_socialComments']
    return row

days = [datetime(2015, 10, d) for d in range(1,11)]
